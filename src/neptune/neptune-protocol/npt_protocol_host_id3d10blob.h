                                         ...
                                         uint64_t object_id)
{
    ...
    if (!args._self) {
        ...
        if (!(cmd_flags & NPT_CMD_FLAG_REPLY)) {
            npt_log("dropping ID3D10Blob_GetBufferPointer on unregistered object "
    ...
    if (npt_cs_decoder_take_handle_miss(ctx->decoder)) {
         ...
         * when something does. */
        if (!(cmd_flags & NPT_CMD_FLAG_REPLY)) {
            npt_log("dropping ID3D10Blob_GetBufferPointer: an argument handle is unknown "
                    "to this context");
            npt_cs_decoder_reset_temp_pool(ctx->decoder);
            return;
        }
                                      ...
                                      uint64_t object_id)
{
    ...
    if (!args._self) {
        ...
        (void)npt_cs_decoder_take_handle_miss(ctx->decoder);
        if (!(cmd_flags & NPT_CMD_FLAG_REPLY)) {
            npt_log("dropping ID3D10Blob_GetBufferSize on unregistered object "
                    "0x%016" PRIx64, (uint64_t)object_id);
            npt_cs_decoder_reset_temp_pool(ctx->decoder);
            return;
        }
    ...
    if (npt_cs_decoder_take_handle_miss(ctx->decoder)) {
         ...
         * when something does. */
        if (!(cmd_flags & NPT_CMD_FLAG_REPLY)) {
            npt_log("dropping ID3D10Blob_GetBufferSize: an argument handle is unknown "
                    "to this context");
            npt_cs_decoder_reset_temp_pool(ctx->decoder);
            return;
        }
